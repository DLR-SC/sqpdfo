   ...
   while nfree > 0:
      ...
      if (len(out_of_ubound) + len(out_of_lbound) != 0):
         ...
         if (len(out_of_ubound) > 0):
            delta_b = min(array([abs(ub[0, ind_free[out_of_ubound]] - s[0, ind_free[out_of_ubound]])]))

         if (len(out_of_lbound) > 0):
            delta_b = min(array([abs(lb[0, ind_free[out_of_lbound]] - s[0, ind_free[out_of_lbound]]) ]))
            if (len(out_of_ubound) > 0):
               delta_b = min(min(array([abs(ub[0, ind_free[out_of_ubound]] - s[0, ind_free[out_of_ubound]])])), delta_b)
         ...
         while (((len(ind_u_active) + len(ind_l_active)) == 0) or (len(out_of_lbound) + len(out_of_ubound) != 0)):
            ...
            if (p == 0 and hardcase == 0):
               ...
               s_duringH = s + dot(s_deltaH, I[:, ind_free].T)
               ...
               if (stratLam == 1 or verbose > 0):
                  ...
                  if (logical_or(len(out_of_ubound) != 0, len(out_of_lbound) != 0)):
                     ...
                     if (len(out_of_ubound) != 0):
                        diff_b_u = max(array([abs(ub[0, ind_free[out_of_ubound]] - s_duringH[0, ind_free[out_of_ubound]])]))
                     ...
                     if (len(out_of_lbound) != 0):
                        diff_b_l = max(array([abs(s_duringH[0, ind_free[out_of_lbound]] - lb[0, ind_free[out_of_lbound]])]))
               ...
               if (len(out_of_uEpsbound) + len(out_of_lEpsbound) == 0):
                  ...
                  if ((len(ind_u_active) + len(ind_l_active)) != 0):
                     ...
                     s_afterH = s + dot(s_deltaH, I[:, ind_free].T)
