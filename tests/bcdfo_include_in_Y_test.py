...
from runtime import matlabarray, compare_matlabarray, char
...
#        TEST:
   ...
   [      0,         0,         0,         0,         0,    -2.425356250363330e-01]])
    ...
    [0.2500],])
        ...
        #print "QR decompositions is different"
#        print QZ
#        print RZ
        ...
        var=np.dot(helper.convert(QZplus.T), bcdfo_evalZ((helper.convert((Yplus)-np.dot(helper.convert(Y[:,1].T), np.ones((1,6))))*helper.convert(scale[2])),6))
        ident=np.linalg.solve(var,helper.convert(RZplus))
